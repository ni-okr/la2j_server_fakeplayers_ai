...
class LSTMLayerTest {
    ...
    @BeforeEach
    void setUp() {
        layer = new LSTMLayer(HIDDEN_UNITS);
    ...
    class LayerCreationTests {
        ...
        @Test
        @DisplayName("Should create LSTM layer with correct parameters")
        void testLayerCreation() {
            ...
            assertEquals(NetworkLayer.LayerType.LSTM, layer.getType(), "Should have correct type");
        }
        ...
        void testInvalidHiddenUnits() {
            assertThrows(IllegalArgumentException.class, () -> new LSTMLayer(0), 
                "Should throw exception for zero hidden units");
            assertThrows(IllegalArgumentException.class, () -> new LSTMLayer(-1), 
    ...
    class StateManagementTests {
        ...
        void testSetInitialState() {
            ...
            double[] initialCell = createTestInput(HIDDEN_UNITS);
            
            layer.setInitialState(initialHidden, initialCell);
            
            // Проверяем, что состояние установлено (нет исключений)
            assertDoesNotThrow(() -> layer.setInitialState(initialHidden, initialCell), 
        ...
        void testSetNullInitialState() {
            assertDoesNotThrow(() -> layer.setInitialState(null, null), 
        ...
        void testSetInvalidInitialStateSize() {
            ...
            double[] invalidCell = createTestInput(HIDDEN_UNITS + 1);
            
            assertDoesNotThrow(() -> layer.setInitialState(invalidHidden, invalidCell), 
    ...
    class StateAccessTests {
        ...
        @Test
        @DisplayName("Should get cell state")
        void testGetCellState() {
            double[] input = createTestInput(INPUT_SIZE);
            layer.forward(input);
            
            double[] cellState = layer.getCellState();
            
            assertNotNull(cellState, "Cell state should not be null");
            assertEquals(HIDDEN_UNITS, cellState.length, "Should have correct cell state size");
        }
    ...
    class StatisticsTests {
        ...
        void testStatistics() {
            ...
            assertTrue(stats.contains("LSTMLayer Statistics"), "Should contain layer type");
